...
namespace CloudLoadTestingClient
{
    class CltUploadDownloadHelper
    {
        ...
        /// <summary>
        /// Downloads the diagnostic data from the given uri and relative path to the target file path. In case target file path is not specified
        /// downloads to a default location and returns the file path.
        /// </summary>
        /// <param name="diagnosticUri">uri of the results file</param>
        /// <param name="relativePathToDiagnosticFiles">relative path of the diagnostic files </param>
        /// <param name="targetFilePath">target file path to which the file will be downloaded</param>
        /// <returns>file location of the downloaded file</returns>
        public static string DownloadDiagnosticData(string diagnosticsUri, string relativePathToDiagnosticFiles = null, string targetFilePath = null)
        {
            if (string.IsNullOrWhiteSpace(diagnosticsUri))
            {
                throw new ArgumentException("Invalid diagnostics uri.", "diagnosticsUri");
            }

            var container = new CloudBlobContainer(diagnosticsUri);
            var blobDirectory = container.GetDirectoryReference(relativePathToDiagnosticFiles);
            var blobFiles = blobDirectory.ListBlobs(new BlobRequestOptions { UseFlatBlobListing = true }).ToList();

            var filePathToUse = string.IsNullOrWhiteSpace(targetFilePath) ? ResultsFileLocation : targetFilePath;          
            foreach (var listBlobItem in blobFiles)
            {
                var blob = (CloudBlob)listBlobItem;
                var path = filePathToUse + "/" + blob.Name;
                Directory.CreateDirectory(Path.Combine(filePathToUse, Path.GetDirectoryName(blob.Name)));
